...
public class Board {
	public static String VRESION = "0.4.0";
...
//		Color.rgb(93, 14, 172, "글 등록이 완료되었습니다.");
				String sql;
				sql = String.format("insert into board (b_title,b_id,b_datetime,b_text,b_hit)" + "values ('%s','%s',now(),'%s',0);" ,title,id,con);
				System.out.println("SQL로그:"+sql);  // <- 로그는 꼭 찍는것을 추천 나중에 어디서 걸렸는지 알기 쉬움
				dbExecuteUpdate(sql);
				break;
			case"4":
