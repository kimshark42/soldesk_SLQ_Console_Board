...
public class Db {
	...
	// 로그인 처리
	public static boolean isSiteLogin(String id, String pw) {
		String count = "";
		try {
			//select count(*)from member where s_id='qwer' and s_pw='1234';
			Db.result = Db.st.executeQuery(
					"select count(*) from member where s_id='"
					+id+"' and s_pw='"+pw+"'");
			Db.result.next();
			count = Db.result.getString("count(*)");
			Color.sum(Color.BG_BLACK, "찾은 회원 수:"+count);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(count.equals("1")) {
			Color.sum(Color.BG_BLACK, "로그인에 성공하였습니다");
			return true;  // 로그인 성공
		} else {
