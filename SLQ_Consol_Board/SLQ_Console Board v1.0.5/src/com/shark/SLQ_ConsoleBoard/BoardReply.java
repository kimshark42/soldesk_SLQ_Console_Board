...
import com.shark.SLQ_ConsoleBoard.site.SiteMain;
...
public class BoardReply {
	...
	static public void list(int oriNo) {  // 댓글 리스트 출력
		...
		try {
			...
			while(Db.result.next()) {
				...
				String b_id = Db.result.getString("b_id");  // 댓글 작성자 id 도 넣기
				Cw.wn(b_reply_text+"-"+b_id);  // 댓글에 작성자 id도 표시
			}
	...
	static public void write(int b_reply_ori) {  // 댓글 작성 함수
		...
		Db.dbExecuteUpdate(
		"insert into board (b_id,b_datetime,b_reply_ori,b_reply_text) values("
		+ SiteMain.loginedld +"',now(),"+b_reply_ori+","+b_replay_text+"')");
	}
}
